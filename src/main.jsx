...
// --- 🐛 DEBUGGING LOG 🐛 ---
// Open your browser console (F12). If you don't see this message, 
// index.html is NOT loading this file. Check your file paths!
console.log("🚀 CaloMad: Main script is running...");

// --- 🔴 GLOBAL ERROR TRAP 🔴 ---
window.onerror = function(message, source, lineno, colno, error) {
  const errorDiv = document.createElement('div');
  errorDiv.style.cssText = 'position:fixed;top:0;left:0;width:100%;height:100%;background:white;color:red;z-index:9999;padding:20px;overflow:auto;font-family:monospace;';
  errorDiv.innerHTML = `
    <h2 style="color:darkred">⚠️ Application Crashed</h2>
    <p><strong>Error:</strong> ${message}</p>
    <p><strong>Location:</strong> ${source}:${lineno}</p>
    <pre style="background:#eee;padding:10px;border-radius:5px">${error?.stack || 'No stack trace'}</pre>
  `;
...
const analyzeImageWithGemini = async (base64Image) => {
  if (GEMINI_API_KEY.includes("YOUR_GEMINI")) {
    alert("Missing Gemini API Key! Please add it in main.jsx");
...
// ⚠️ MOUNT POINT: This checks if 'root' exists before trying to render
const rootElement = document.getElementById('root');
if (rootElement) {
  const root = ReactDOM.createRoot(rootElement);
  root.render(<App />);
} else {
  console.error("FATAL ERROR: Could not find <div id='root'> in index.html");
  document.body.innerHTML = '<div style="color:red;padding:20px;">Error: Missing root element</div>';
}